(() => {
  ...
  function compare(curr, prev, invert = false) {
    ...
    if (!prev) return "sem base anterior";
  ...
  function renderGeneral() {
    ...
    renderSummary("general-summary", [
      { label:"Tempo estudado", value:hours(m.seconds), helper:compare(m.seconds,m.prevSeconds), progress:Math.min(100,m.seconds/108000*100) },
  ...
  function renderLessons() {
    ...
    renderSummary("lesson-summary", [
      ...
      { label:"Pontualidade", value:percent(m.onTime,1), helper:`${m.lateDelay.length} após o prazo`, progress:m.onTime },
      ...
      { label:"Dias com aula", value:num(studyDays), helper:`${percent(pct(studyDays,eligibleDays(state.settings?.theory_study_weekdays)))} dos permitidos`, progress:pct(studyDays,eligibleDays(state.settings?.theory_study_weekdays)) },
  ...
  function renderFlashcards() {
    ...
    renderSummary("flash-summary", [
      ...
      { label:"Novos cards", value:num(m.created.length), helper:"criados", progress:Math.min(100,m.created.length/300*100) },
      ...
      { label:"Média por dia ativo", value:m.reviewDays?num(m.reviews.length/m.reviewDays,1):"0", helper:"revisões/dia", progress:Math.min(100,(m.reviewDays?m.reviews.length/m.reviewDays:0)/150*100) },
  ...
  function wire() {
    ...
    document.querySelectorAll("[data-range]").forEach(btn=>{
      btn.addEventListener("click",async()=>{
        ...
        if(![15,30,90,"all"].includes(value)||value===state.range) return;
