...
async function salvarAula() {
  ...
  if (!data || !area || !tema) {
    mensagem.textContent = "Escolha a área e preencha a data e o tema da aula.";
    return;
  }
        ...
        <div class="cronograma-item">
          ...
          ${
            prova.data_limite_inscricao
              ? `<p>Inscrição até: ${formatarData(prova.data_limite_inscricao)}</p>`
              : ""
          }
          ${
            prova.data_divulgacao_gabarito
              ? `<p>Gabarito em: ${formatarData(prova.data_divulgacao_gabarito)}</p>`
              : ""
          }
        </div>
      `
...
async function salvarFlashcard() {
  ...
  if (!frenteInput || !versoInput || !imagemInput || !mensagem) return;
...
async function atualizarTelaEstudo() {
  ...
  if (!frente || !verso || !imagem || !card || !resposta || !botaoVirar) return;
