      ...
      >
        ...
        <div class="sidebar-lofi-head">
          ...
          <div class="sidebar-lofi-copy">
            ...
            <strong data-lofi-title>
              Lo-fi DocMap
            </strong>

            <small data-lofi-status>
              carregando...
            </small>
...
function prepararConfiguracoes() {
  ...
  const radio =
    form.querySelector(
      `input[name="theme"][value="${currentThemeSetting}"]`
    );
...
async function carregarAudioPreferido(
  userId
) {
  const {
    data,
...
async function salvarAudioPreferido(
  userId,
  trackId
) {
  const {
      ...
      select.innerHTML =
        manager.tracks.length
          ? manager.tracks
              .map(
                (track) => `
                  <option value="${escapeHtml(track.id)}">
...
async function iniciarLofiGlobal(
  userId
) {
  const [
    tracks,
...
async function iniciarApp() {
  ...
  iniciarLofiGlobal(
    ...
    console.warn(
      "Não foi possível iniciar o player de áudio:",
      error
    );
  });
